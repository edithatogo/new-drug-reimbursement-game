...
This research script generates comparative scenario sweeps across:
1. Chapter 7 Scenarios 1-4 (Expandable vs Fixed Budgets and Technical Investment)
2. Chapter 8 Strategic Pricing Equilibria & Bargaining Rent Allocation
3. Research Extensions (Managed Access Agreements and Value of Evidence)
"""
...
from reimbursement_game.application_games import solve_game1_bargaining
...
def generate_chapter7_comparison_figure(output_path: Path) -> None:
    ...
    ax.set_xlabel("Incremental Cost (\\textsterling)", fontsize=10)
...
def generate_game_equilibria_figure(output_path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 5), dpi=300)
    ax.plot(bargaining_shares, firm_rents, label="Manufacturer Economic Rent (\\textsterling)", color="#9467bd", linewidth=2)
    ax.plot(bargaining_shares, institution_benefits, label="Health System Net Benefit (\\textsterling-equiv)", color="#17becf", linewidth=2)
    ...
    ax.set_ylabel("Surplus Value (\\textsterling)", fontsize=10)
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(frameon=True, fontsize=9)
    plt.tight_layout()
...
def generate_ced_valuation_figure(output_path: Path) -> None:
    ...
    ax.set_ylabel("Effective Net Settlement Price (\\textsterling)", fontsize=10)
    ax.grid(True, linestyle="--", alpha=0.5)
    ax.legend(frameon=True, fontsize=9)
    plt.tight_layout()
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("docs/figures"), help="Target figure output directory")
