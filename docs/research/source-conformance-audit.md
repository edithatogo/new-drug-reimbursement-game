...
surface. The audit is bound to the authorized Pekarsky PDF with SHA-256
...
| Scenario 4 `mu`, `beta_c^v` | printed p. 117/PDF p. 127 | Excluded; never mapped to generalized productivity | exclusion documentation |
...
Exact Game 1 validation enforces positive fixed health effect, fixed budget,
